...
const LabelsLayoutConfigurator = ({ orientation = 'portrait', onLayoutChange, labelData = [] }) => {
  ...
  const [labelStyle, setLabelStyle] = useState({
    ...
    nameSize: 10,
    duplicateCount: 1, // ✅ NOUVEAU : Nombre d'étiquettes par produit
  });
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => handleStyleChange({ duplicateCount: parseInt(e.target.value) || 1 })}
              className="w-full px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded focus:ring-1 focus:ring-blue-500 bg-white dark:bg-gray-700"
          ...
          <div className="flex items-center">
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded p-3 w-full">
              ...
              <div className="text-lg font-bold text-blue-600 dark:text-blue-400">
                {labelData.length} × {labelStyle.duplicateCount} ={' '}
                {labelData.length * labelStyle.duplicateCount}
              </div>
              <div className="text-xs text-blue-700 dark:text-blue-300">
                {labelData.length} produit{labelData.length > 1 ? 's' : ''} sélectionné
                {labelData.length > 1 ? 's' : ''}
              </div>
            </div>
          </div>
        </div>
